...
export type royalties_key = ex.Nat;
...
export const royalties_key_mich_type: ex.MichelineType = ex.prim_annot_to_mich_type("nat", []);
...
export const royalties_container_mich_type: ex.MichelineType = ex.pair_to_mich_type("big_map", ex.prim_annot_to_mich_type("nat", []), ex.list_annot_to_mich_type(ex.pair_array_to_mich_type([
    ex.prim_annot_to_mich_type("address", ["%part_account"]),
    ex.prim_annot_to_mich_type("nat", ["%part_value"])
]), []));
...
const set_permits_arg_to_mich = (p: ex.Address): ex.Micheline => {
    return p.to_mich();
}
...
const transfer_gasless_arg_to_mich = (batch: Array<gasless_param>): ex.Micheline => {
    return ex.list_to_mich(batch, x => {
        return x.to_mich();
    });
}
...
const mint_arg_to_mich = (tow: ex.Address, tid: ex.Nat, tmd: Array<[
    string,
    ex.Bytes
]>, roy: Array<part>): ex.Micheline => {
    return ex.pair_to_mich([
        tow.to_mich(),
        tid.to_mich(),
        ex.list_to_mich(tmd, x => {
            const x_key = x[0];
            const x_value = x[1];
            return ex.elt_to_mich(ex.string_to_mich(x_key), x_value.to_mich());
        }),
        ex.list_to_mich(roy, x => {
            return x.to_mich();
        })
    ]);
}
const burn_arg_to_mich = (tid: ex.Nat): ex.Micheline => {
    return tid.to_mich();
}
...
const view_get_royalties_arg_to_mich = (tokenId: ex.Nat): ex.Micheline => {
...
export class Fa2nft {
    ...
    async mint(tow: ex.Address, tid: ex.Nat, tmd: Array<[
        string,
        ex.Bytes
    ...
    async burn(tid: ex.Nat, params: Partial<ex.Parameters>): Promise<any> {
        if (this.address != undefined) {
    ...
    async get_set_permits_param(p: ex.Address, params: Partial<ex.Parameters>): Promise<ex.CallParameter> {
        if (this.address != undefined) {
    ...
    async get_transfer_gasless_param(batch: Array<gasless_param>, params: Partial<ex.Parameters>): Promise<ex.CallParameter> {
        if (this.address != undefined) {
    ...
    async get_mint_param(tow: ex.Address, tid: ex.Nat, tmd: Array<[
        string,
        ex.Bytes
    ]>, roy: Array<part>, params: Partial<ex.Parameters>): Promise<ex.CallParameter> {
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "mint", mint_arg_to_mich(tow, tid, tmd, roy), params);
        }
        throw new Error("Contract not initialised");
    }
    async get_burn_param(tid: ex.Nat, params: Partial<ex.Parameters>): Promise<ex.CallParameter> {
        if (this.address != undefined) {
    ...
    async view_get_royalties(tokenId: ex.Nat, params: Partial<ex.Parameters>): Promise<Array<part>> {
        if (this.address != undefined) {
            ...
            for (let i = 0; i < mich.length; i++) {
                res.push((x => { return new part((x => { return new ex.Address(x); })(x.part_account), (x => { return new ex.Nat(x); })(x.part_value)); })(mich[i]));
            }
            return res;
        }
        throw new Error("Contract not initialised");
    ...
    errors = {
        fa2_r7: ex.string_to_mich("\"CALLER_NOT_OWNER\""),
        fa2_r6: ex.pair_to_mich([ex.string_to_mich("\"INVALID_CONDITION\""), ex.string_to_mich("\"fa2_r6\"")]),
        FA2_TOKEN_UNDEFINED: ex.string_to_mich("\"FA2_TOKEN_UNDEFINED\""),
        ...
        fa2_r3: ex.pair_to_mich([ex.string_to_mich("\"INVALID_CONDITION\""), ex.string_to_mich("\"fa2_r3\"")]),
        ...
        p_r1: ex.pair_to_mich([ex.string_to_mich("\"INVALID_CONDITION\""), ex.string_to_mich("\"p_r1\"")]),
